-- 0077_privacy_article_finalize.up.sql — chore/analytics-legal-copy-finalized.
-- Removes the DRAFT blockquote from the `privacy-and-tracking` article body now
-- that legal review is complete (0076 is shipped/immutable, so this is a follow-on
-- UPDATE rather than an edit to 0076). Body structure/sections unchanged.

UPDATE help_schema.help_articles
SET body_translations = jsonb_build_object(
  'tr', E'## Hangi verileri topluyoruz?\n\nGörüntülediğin ürünler, aradığın kelimeler (yalnızca normalleştirilmiş haliyle), '
