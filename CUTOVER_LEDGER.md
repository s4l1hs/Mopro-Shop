...
| Search | **PROBABLE resolved source-side** (`feat/search-probable-resolution`): **SE-08 relevance ranking = CONFIRMED ✅ FIXED** (ts_rank default sort, DISTINCT dropped via the (product_id,locale) PK; discovery shift — was id-ordered on main, not already done). SE-10 sponsored = NOT-ACTIONABLE (no ads model); SE-10 search-within = NEEDS-DECISION. Doc: `docs/internal/search-probable-resolution.md`. |
