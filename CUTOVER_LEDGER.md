...
- **Remaining (PR 4 — FilterPanel accordion):** `FilterPanel`/`PlpFilterSheet` `renk` accordion (mirror `PlpBrandFacet`, driven by the facet buckets, search inherits) + `PlpFilters.attrs`/codec/chips + a facets provider + provider wiring (thread `attr`) + goldens. `depolama`/other attrs + Phase 2/3 deferred per #149. Phase-1 plan: `docs/internal/plp-13-p1.md`.
