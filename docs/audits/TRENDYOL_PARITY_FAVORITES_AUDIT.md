...
| **FAV-07** | ~~No favorites-specific price-drop indicator~~ → **✅ RESOLVED** (`feat/fav-07`): a "fiyatı düştü −%X" pill on each favorites card when the **live** price (from `/products/batch`) is below the **price-at-favorite snapshot**. Discovery shift — **FE-only**, not the assumed backend/data: favorites are device-local (`Set<int>`; server holds only IDs via the cart-owned sync), so the snapshot is captured client-side at heart-toggle (`{productId: priceMinor}` in `SharedPreferences`) — works for guest + authed, no migration/codegen. Pre-FAV-07 favorites carry no snapshot → no badge (graceful; nothing fabricated). In the favorites-local wrapper, not the shared `ProductCard` (§3). | src | **RESOLVED** | LOW–MED |
