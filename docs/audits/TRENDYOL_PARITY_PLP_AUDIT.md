...
| **PLP-13** 🆕 | **no attribute/variant facets** → Trendyol's deep stack. **Phase 1 STARTED (`feat/plp-13-attribute-model-p1`)**: the normalized model now exists — migration 0089 `attribute_keys`/`category_facets`/`product_attributes` (§5-safe) + a `renk` (colour) backfill from `variants.color` (`attr-extras.sql`), live-verified. **Foundation landed; staged:** PR 2 = facet endpoint + `attr` filter + `Product.attributes` + codegen; PR 3 = `FilterPanel` accordion (search inherits) + PDP specs tab (PD-01). `depolama`/other attrs + later phases deferred per #149. Design `docs/internal/plp-13-attribute-model.md`; phase-1 plan `docs/internal/plp-13-p1.md`. | markup | **CONFIRMED → in progress (P1 foundation)** | **HIGH** (backend) |
