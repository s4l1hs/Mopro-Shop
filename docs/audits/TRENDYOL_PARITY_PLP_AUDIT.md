...
- **PROBABLE → resolved source-side** (`feat/plp-probable-resolution`, no walk; `docs/internal/plp-probable-resolution.md`): **PLP-08** no-results clear-filters CTA → **CONFIRMED ✅ FIXED** (`EmptyState.filtered` + `CatalogShell.onClearFilters` + `PlpFiltersNotifier.clear()`, i18n + test). **PLP-06** quick-filter pills → ~~NEEDS-DECISION~~ **✅ RESOLVED** (`feat/browse-search-ux`: `PlpQuickPills` — Ücretsiz Kargo / Fiyatı Düşenler / 4★+ / Stokta, mobile-only, wired to `plpFiltersProvider`). **PLP-07** brand-facet counts → **NEEDS-VISUAL** (Trendyol-side inconclusive — counts not in SSR). **PLP-10** inline PLP search bar → ~~NEEDS-DECISION~~ **✅ RESOLVED** (`feat/browse-search-ux`: `PlpInlineSearchField` + `plpInlineQueryProvider` — non-empty query swaps the grid source to the category-scoped `/search` with identical filters/pagination; zero backend/codegen). §9 bucket → **NEEDS-VISUAL**. (PLP-02 was remapped to PLP-18, already resolved.)
...
| **PLP-17** 🆕 | ~~official-seller badge ("Resmi satıcı") — Mopro has none~~ → **RESOLVED** (`feat/plp-17-official-seller`): migration 0090 adds `seller_schema.sellers.is_official` (sellers 1,3 seeded official); surfaced §5-safely via `seller.Service.OfficialSellerIDs` (handler app-merge per page — no cross-schema JOIN) → `ProductSummary.is_official_seller` → blue "Resmi Satıcı" card badge. Two-surface win with **PD-04**. | markup | **RESOLVED** | LOW |
