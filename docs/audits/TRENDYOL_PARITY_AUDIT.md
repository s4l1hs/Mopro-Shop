...
`feat/bestseller-unhide`; category-scope → **P-031**). **P-030 price-history ✅ RESOLVED on cards**
(backend `feat/price-history` Mechanism B trigger + `lowest_30d_price_minor`; card display
`feat/lowest-30d-display`; PDP display backend-blocked → deferred; price-update lifecycle → **P-032**).
**The pure-UI parity work is done.** Remaining (all backend/architectural): P-007 (delivery-ETA), P-031
(category bestseller), P-032 (price-update lifecycle), P-030-PDP (backend-gated display), chi-square flake.**
...
**Status: ✅ RESOLVED on cards (backend Mechanism B + card display); PDP display backend-blocked (deferred) | Severity: HIGH | Confidence: CONFIRMED | Type: backend + frontend / compliance**
