...
| — | Summary: subtotal, cargo, coupon, "Sepette indirim", coin, **total** | desktop `OrderSummaryCard`; mobile `CartTotalsSummary` | **CT-04 ✅ RESOLVED**; **CT-09 DEFER (CFO/ADR-gated)** — making `basket_discount_pct` real is a **7-module financial refactor** (pricing→payment→order→cashback→payout→ledger) whose **funding model (seller- vs Mopro-funded) is an undefined CFO decision** changing **v6-LOCKED** cashback/payout inputs (§4.7/§12) → can't land safely without an ADR; partial-apply barred. **Trust gap flagged** (pill advertises an uncharged discount). Plan: `docs/internal/basket-discount-pricing.md`. | **CT-04 RESOLVED / CT-09 DEFER** | MED |
