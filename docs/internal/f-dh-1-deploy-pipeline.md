...
|---|---|---|---|
| ① | bare `docker compose` (no `-f`) → dev compose wins | **CONFIRMED** | `deploy_script.sh` `dc() { sudo docker compose "$@"; }`; host `/opt/mopro/deploy/` holds BOTH `docker-compose.yml` and `docker-compose.prod.yml`; bare compose auto-loads `docker-compose.yml`. Run 27087000549's `.env.local` warnings + `ghcr.io/mopro/*` pulls match the dev file, not prod. |
