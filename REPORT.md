...
## PR — Favorites UX: FAV-05 add-to-cart + FAV-06 sort/filter (`feat/favorites-ux`) — **shipped (local-verify; no deploy)**
Closes the two deferred favorites rows. **FAV-05 add-to-cart from the card:** favorites store product IDs only, so the tap resolves variants client-side via the **existing `GET /products/{id}` read path** (the PDP's; already §5-clean) — **no backend change, NO codegen** (the lane's anticipated catalog carrier wasn't needed; discovery shift #1). Resolution rule: 0 in-stock variants → `favorites.out_of_stock` snackbar; **exactly 1 in-stock** (incl. multi-variant with one purchasable) → direct `cartProvider.addItem(qty:1)` + the PDP's added-to-cart snackbar w/ go-to-cart; **several in-stock** → PDP redirect for explicit size/colour (discovery shift #2: correctness over auto-pick; quick-add sheet = future polish). The button lives in a **favorites-local `_FavCard` wrapper** — the shared `ProductCard` is untouched, dodging the §3 serialization the probable-resolution audit flagged (shift #3); per-card in-flight spinner state. **FAV-06 sort/filter:** client-side toolbar over the fetched list — sort popup (default fetch order / price ↑ / price ↓ / deepest discount; reuses `catalog.sort_*`) + toggle chips (discounted, free shipping; reuses `plp.free_shipping`); screen-local autoDispose state; filter-empty hint distinct from the true empty state; in-stock NOT filterable (`ProductSummary` has no stock — FAV-07 residue). i18n: 6 new `favorites.*` keys (tr master + en; de/ar lack the section entirely, strict gate counts extra-only). Tests: 14 (incl. OOS / direct-add / multi-variant-redirect / 404-failure / sort-reorder / chip-prune / filter-empty) via a route-faking Dio adapter + recording cart repo + GoRouter harness. Favorites **goldens flip** (toolbar + button) — Linux-baselined, regen via the informational rebaseline workflow. Audit updated (FAV-05/06 → RESOLVED; remaining: FAV-07 price-drop snapshot, FAV-01 collections). Discovery: `docs/internal/favorites-ux.md`. Local-verify only; no deploy.
