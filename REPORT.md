...
## PR — Coupon apply/validate (`feat/coupon-discount`) — **CT-03 / CHK-04 RESOLVED (seller-funded, Salih-confirmed)**
`docs/internal/coupon.md` + migration `0092`: the cart coupon is now real. **Discovery-first (financial path):** no coupon backend existed (only an inert desktop `OrderSummaryCard` placeholder); the **funding model was the crux** and was confirmed **seller-funded** (Option A — mirrors CT-09; Option B platform-funded would have tripped `CLAUDE.md §12` → DEFER+ADR). **Mechanism reused = CT-09's `order/pricing.go` `DiscountedUnitMinor`:** a percent coupon is applied **per unit, on top of** the basket discount, so `order_items.unit_price_minor` stays the single charged-unit snapshot every consumer derives from → commission/KDV/seller-net **and cashback** all compute on the coupon-discounted price (the snapshot does the work; **fin-svc untouched, no new ledger account, capture ledger balances**). **Backend:** `order_schema.coupons` + `coupon_redemptions` (UNIQUE(coupon_id, order_id) ⇒ idempotent redemption, financial-core §4); `orders.coupon_code`/`coupon_discount_minor`; `order.Service.ValidateCoupon` (read preview); `Checkout` + `InitiateCheckout` (saga) two-pass build (resolve coupon vs basket-discounted subtotal → apply per unit → record redemption in-tx). **Display==charge:** `GET /cart?coupon=CODE` (`enrichCart`) runs the same resolve logic; `/checkout/initiate` + legacy `/orders/checkout` carry `coupon_code`. **Mobile:** CartDto coupon fields, `CartNotifier.applyCoupon`, OrderSummaryCard apply button (was inert) + coupon line + invalid message, checkout sends the code; i18n `cart.coupon_discount`/`cart.coupon_invalid`. **Edge:** stale/expired code at charge time silently dropped (charge full = buyer-safe); multi-seller cart coupon records one redemption per seller-order (conservative). **v1 = percent, cart-level;** fixed-amount (largest-remainder distribution), admin/creation, and richer scope/stacking deferred. **Tests:** coupon guards + coupon-stacks-on-basket-discount math + cart-enrich display==charge + invalid-coupon no-op; existing order/cart/checkout suites green (fakes updated). Goldens (cart) show the known host-vs-Linux diff (empty-cart fails too → unrelated), informational. Audits: CT-03 (cart) + CHK-04 (checkout) → RESOLVED; ledger note folded into `basket-discount-pricing.md` §6. Local-verify; no deploy.
