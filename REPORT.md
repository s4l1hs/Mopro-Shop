...
## PR — Trendyol Cart parity audit (`docs/trendyol-parity-cart-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_CART_AUDIT.md`: parity audit of Cart (`CartScreen` + `CartLineCard` + `CartTotalsSummary`/`OrderSummaryCard` + `EmptyCart` + `cartProvider`/`guestCartProvider`/`cartMergeService`). **Audit-only — no code.** **Cart is well-built:** per-seller grouping, qty stepper, swipe+button remove, desktop price summary (subtotal/shipping/cashback/total + KDV-included), coin cashback line, auth-gated checkout CTA, empty state, **guest cart + guest→auth merge**, stock reservation (`reservedUntil`). **CONFIRMED gaps (src):** **CT-01** seller group shows `#<sellerId>` not name + no per-seller subtotal (though `totalsBySeller` carries it); **CT-02** no free-shipping progress; **CT-03** coupon is a desktop-only **placeholder** (no backend); **CT-04** mobile summary lacks the subtotal/shipping breakdown desktop has; **CT-05** line has no variant label (`CartLineDto` has `variantId` only) / no save-for-later / no move-to-favorites; **CT-06** no stock/price-changed warnings; **CT-07/08** no recommendations (empty + populated); **CT-09** no basket-discount "Sepette indirim" line (cross-ref PD-03); **CT-10** "remove" snackbar has no real Undo action. **4 NOT-ACTIONABLE** (coin/cashback line, brand tokens, the `isAboveTotalLimit` total-limit guardrail, checkout auth-gating). **Seed/gating:** cart **populatable locally** — the guest cart persists to SharedPreferences (`mopro_guest_cart`) + add-to-cart from the PDP; checkout is auth-gated (`requireAuth`→`LoginRequiredSheet`). **Cascade (§5/§10):** the line-item + `totalsBySeller`/`grandTotal`/KDV/cashback model is the **Checkout** foundation — CT-01/CT-04 pay forward; CT-03/CT-09 are shared backend prereqs. Walk slots (#09). No deploy.
