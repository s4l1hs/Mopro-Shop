...
## PR — PDP UX: sticky buy-box + recently-viewed + mobile thumbs (`feat/pdp-ux`, lane A1) — **shipped (local-verify; no deploy)**
Closes the three decided PDP UX gaps — all UI on existing data, **no codegen, no shared-widget internals** (`ProductCard`/`FilterPanel` untouched; rails mount `ProductCard` read-only via the existing `ProductListRail`). **(1) PD-09 sticky buy-box:** new **`PdpStickyBuyBar`** (thumbnail + title + price + add-to-cart) slides in at the viewport top on the **wide** layout once the buy-box column scrolls out of view — driven by the already-tracked `_scrollOffset`/`_buyBoxHeight` signals; the **locked sticky-gallery translate mechanics are untouched** (the audit flagged pinning the buy-box *instead* as a UX redesign — this adds the affordance without it; mobile already pins via `PdpStickyCta`). The bar is **`Offstage` while hidden**, so the desktop composition test's "exactly one ATC at scroll origin" assertion and the scroll=0 `pdp_two_col_*` goldens are unaffected; behavior locked by `pdp_sticky_buy_bar_test.dart` (hidden at top → appears past the buy-box → hides again). **(2) PD-10 recently-viewed rail:** `_RecentlyViewedRail` mounts the existing `recentlyViewedProvider` (auth+consent gated; error→empty) via `ProductListRail` — wide below the similar rail, mobile in the description tab — **filtering out the product being viewed**; zero space when empty/ungated; reuses `home.rails.recently_viewed.title` (no new i18n keys, **i18n 0/0**). Locked by `pdp_recently_viewed_test.dart` (renders + excludes current product + hides when empty/self-only). **(3) PD-06 mobile thumbnail strip:** `PdpImageGallery` now overlays a tappable 48dp thumbnail strip (tap → `animateToPage`; active thumb highlighted, swipe-synced via a `PageController` listener), **replacing the worm dots**; single-image galleries render no strip; desktop `PdpImagePager` (which already had thumbs) untouched; `smooth_page_indicator` stays (home banner carousel). Locked by `pdp_image_gallery_test.dart`. **Gates:** changed files analyze clean (repo-wide pre-existing infos only); sticky-gallery + composition + new tests green; golden failures local-only (**verified identical on clean `origin/main`** — the documented Linux-baseline vs macOS skew; goldens are the informational CI job, rebaseline on-branch if CI flips). Audit updated (PD-09/PD-10 → RESOLVED; PD-06 thumbs ✅, video stays DEFER). **Discovery shifts:** mobile PD-09 was already satisfied (`PdpStickyCta`) — the gap was desktop-only; `Offstage`-while-hidden is the trick that keeps existing finder-based tests and scroll=0 goldens honest while still animating the entrance. No deploy.
