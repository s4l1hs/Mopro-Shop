...
## PR — Trendyol Returns parity audit (`docs/trendyol-parity-returns-audit`) — **seeded; read-path-checked; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_RETURNS_AUDIT.md`: parity audit of Returns (eligibility → request → status → refund → history) (`returns_list_screen` + `order_return_flow_screen` 4-step + `return_detail_screen` + `RefundStatusCard`/`return_status_chip`; entry via `order_eligibility_actions`). **Audit-only — no code.** Applied the **read-path reality check** (L live / S stub / U UI-only / A absent). **Headline: the request/history/eligibility core is LIVE** — verified to the backend (`handleCreateReturn`/`handleListReturns`/`handleGetReturn` → `order.ReturnService` → `pgxReturnRepository`; seller approve/reject drives status): server-computed eligibility (`ComputeActions` → `canReturn`/`returnableUntil`/`returnableItems`, 14-day window), the 4-step request (idempotent `POST /orders/{id}/returns`), the İadelerim list + paged detail. **The one non-honest finding — RT-01: the refund *settlement* is not wired** — the lifecycle defines `refunded` and `buildReturnRefundView` maps `refunded→issued`, **but no code transitions a return to `refunded` and there's no coin/ledger post or outbox event on approval** (`SellerApprove` stops at `approved`), so the refund card is a "pending" that can never reach "issued" (an **S**: status surface over a dead step). The rest are **honest A-absences**: **RT-02** no return shipping (no cargo code/drop-off/label — confirm's "tracking_no" is just the return id), **RT-03** no return photos, **RT-04** `return_status_history` is written but not surfaced (detail timeline is *derived* from current status, not the audit trail), **RT-06** no consumer status filter. **U/fidelity:** **RT-05** per-item reason UI collapses to the single-reason contract (first item's reason; notes→`description`). **3 NOT-ACTIONABLE** (refund-as-coin `wallet_credit` model — previewed as `method_wallet`; status-derived timeline like Orders; brand tokens). Fix list (§6): RT-01 is a *financial-path lane* (wire `approved→refunded` + the coin refund + outbox), RT-04 is the cheapest (surface existing history rows). Walk slots (#09) §5. **This is the LAST surface audit — the parity surface map is now complete** (Home/PLP/PDP/Search/Cart/Checkout/Orders/Returns/Account/Favorites); remaining is the fix backlog + the deploy/cert call. Awaiting walk. No deploy.
