...
- **T3-3 bootstrap** ✅ — `make bootstrap` → `scripts/bootstrap.sh`: idempotent, repo-local (no sudo), **detects** (never installs) Go/Flutter/Docker, scaffolds `.env`, runs `go mod download` + hooks + `flutter pub get`, lists remaining MANUAL steps. Verified: run 1 all ✅, run 2 idempotent. CONTRIBUTING rewired. (T-005)
...
## PR — Production deploy health report (`chore/deploy-health-report`) — DEPLOY-HEALTH-01: verdict RED, Path-B NO-GO
Read-only audit of deploy run 27087000549 + live VDS. Run is green but **shipped nothing** (`compose pull` denied: script uses the dev compose without `-f`, host compose pins nonexistent `ghcr.io/mopro/*`, no registry auth, no `set -e`) — production runs a 2026-05-26 build, 686 commits stale; 4/5 smoke targets fail solely from staleness (auth gate passes). Infra itself is green: 12/12 containers healthy, 0 app-log errors/7d, 0×5xx, TLS valid to 2026-08-18, govulncheck clean. No remediation performed (nothing down; pipeline fix is outside §3.6 bounds) — ESCALATE F-DH-1 (deploy-pipeline repair) + 7 DEFERs. Full evidence: `docs/audits/PRODUCTION_DEPLOY_HEALTH.md`.

## PR — Deploy-pipeline repair (`fix/deploy-pipeline-f-dh-1`) — closes F-DH-1 (no-op deploys reported green)
Fixes the four stacked defects behind run 27087000549's silent no-op (PR #104 §3): ① `dc()` now pins `-f docker-compose.prod.yml` (bare compose was auto-loading the host's stale **dev** file) and `deploy.yml` scps a fresh prod compose each run; ② namespace default `:-mopro` → `:-s4l1hs` with `IMAGE_NS`/`VERSION` flowing through sudo env passthrough (defect framing CORRECTED in discovery: the repo composes had `${IMAGE_NS}` wired — only the stale host copy hardcoded `ghcr.io/mopro/*`); ③ mandatory GHCR login (`GHCR_USER`/`GHCR_PAT` from host `.env`, `--password-stdin`, packages verified private); ④ `set -euo pipefail` + denied-pull abort + bounded `/healthz` wait + **post-deploy image-ID assertion** (running container == freshly pulled ref; deliberately not "app SHA == deploy ref" since `:latest` legitimately trails on docs-only merges) + deploy summary. Folds F-DH-6 (secrets-file append removed) + F-DH-8 (stale `EXPECTED_SHA_PREFIX` removed). Host prep before next deploy: provision `GHCR_USER`/`GHCR_PAT`; apply migrations (ecom 62→0085, ledger 77→0080) via `apply-migration.sh` BEFORE dispatching. Discovery: `docs/internal/f-dh-1-deploy-pipeline.md`.

## PR — Caddy ACME-renewal repair (`fix/caddy-acme-renewal-f-dh-3`) — closes F-DH-3 (cert renewal DNS errors)
Evidence-driven root cause (discovery doc `docs/internal/f-dh-3-caddy-acme.md`): NOT challenge/creds/persistence — the host's **single upstream resolver (8.8.8.8)** behind Docker's embedded DNS had SERVFAIL episodes (402+94 errors on 06-01/02, tapering to ~5/day; ARI 94-ok/2-fail last 72h) breaking ACME-endpoint lookups from the caddy container. Issuance itself is proven healthy (staging domains obtained 05-26; moproshop.com/www already RE-issued → Aug 24 expiry; no CAA; volumes persisted — discovery shift vs the prompt's re-issue-storm candidate). Fix: `dns: [1.1.1.1, 8.8.8.8, 9.9.9.9]` on the caddy service (one compose line). `caddy validate`: Valid configuration. **Staging-CA exercised through the proposed resolver set on the prod host: 20/20 directory GETs clean** + 10/10 prod-CA reachability (no issuance placed; live cert untouched). Delivery shift: applying = `up -d caddy` container recreate (~2-5s blip), NOT `caddy reload`. RUNBOOK gains a TLS/ACME health section. Earliest cert expiry 2026-08-18; renewal window ~Jul 19.

## PR — Retire legacy tarball deploy path (`chore/deploy-tooling-cleanup-f-dh-residual`) — closes F-DH residual
The root behind prod drifting onto stale local `mopro/*:latest`: `deploy/scripts/{deploy,rollback}.sh` (scp-tarball → `docker load` → tag `mopro/<svc>:latest`; rollback from `bin/prev/`) deleted along with the `release`/`deploy`/`deploy-staging`/`rollback` make targets and their dead `SERVER`/`SSH_PORT` vars; `docker-build` retagged to `ghcr.io/$(IMAGE_NS:-s4l1hs)/*` so **no in-repo tooling tags `mopro/*` anymore**. 9 doc surfaces updated (RUNBOOK deploy/rollback/quirks, launch-day Option B, 2 incident runbooks, DISASTER_RECOVERY §5.4, backups.md + 2 install-script comments) to the canonical workflow path + pinned-`:<full-sha>` GHCR rollback; audit/history docs deliberately untouched. **Host ops:** resolver hardened (`/etc/resolv.conf` 1→3 nameservers — host-wide complement of #106; static file, no provisioner, dhclient-doesn't-manage caveat documented); **image purge STOPPED per §7-4** — live check showed prod STILL on `mopro/*` (re-deploy after #105 hasn't run yet), so the 59-image purge is shipped as a gated RUNBOOK procedure instead ("Post-flip cleanup"). DEFER: `version` input on deploy.yml for dispatch-driven rollback. Discovery: `docs/internal/f-dh-residual-cleanup.md`.

## PR — Revive unwired integration suites (`chore/revive-unwired-integration-suites`) — closes TESTING_AUDIT F-018 (9/10), files F-019
All 10 dark suites triaged **empirically** (each run against real containers; `docs/internal/f018-integration-suites.md`). Discovery shift: no dynamic-port helper needed — the suites' `:6434` env-defaults already point at `pg-ledger-test`; the collisions lived in the LEGACY self-spinning targets (now deleted: `test-integration-{outbox,cart,order,sellerpayout}`). **9 revived** as env-pointer targets on the shared fixtures (cart/identity pattern), all green under `-race`, repeat-stable, registered in `verify`: eventbus, outbox, order, sellerpayout, api(fin), attachments, help, inbox, idempotency. Rot fixed en route (harness-only, assertions untouched): order TestMain DDL + 0059_v8 columns; fin seed + 0076 NOT NULLs (`plans_principal_exact`-satisfying); outbox env overrides. **Second failure class found:** shared-fixture residue — outbox starved by a 71k-row unpublished backlog (fix: TestMain marks stale rows published — UPDATE not TRUNCATE, since wallet's suite counts its fixed-key rows across runs), idempotency replayed a residual derived-key response (fix: FLUSHDB test-DB-15 TestMain). **1 DEFER: reconcile — its revival surfaced a REAL PRODUCTION BUG (filed F-019):** `init/73` grants reconcile_user DELETE without SELECT → `CleanupOldAttempts` throws 42501 every weekly cron run in prod (reproduced live as reconcile_user); needs a 1-line GRANT ledger migration, then batch-2 revival. `verify` runtime +≈45s.

## PR — PDP goldens Linux regen (`chore/pdp-goldens-linux-regen`) — closes GOLD-REGEN (predict-then-verify, 0 flips)
Premise corrected from source: the last regen was the workflow auto-commit `7e1f0a4b` (06-04; PDP set `e050d0b7` 06-03), not "PR #98" (a CI-trigger chore). Prediction (committed BEFORE regen, `docs/internal/gold-regen-prediction.md`): **zero goldens flip** — #97's `PdpDeliveryInfo` is gated on `deliveryEta != null` and #102's strikethrough on `originalPriceMinor != null`, and the PDP golden fixtures omit both (#94-class "fixture doesn't carry the value"); #99 is event-payload-only; #100/#103 touch no mobile code; corroborated by flutter-ci staying green on main (Linux golden comparisons). Dispatched `golden-rebaseline.yml` (run 27102117580): **"No golden changes to commit." — actual ∅ == predicted ∅.** Baselines confirmed in sync; residual documented: `PdpDeliveryInfo`/strikethrough have no golden COVERAGE (fixtures omit the values) — a Path-B PDP-surface item, out of rebaseline scope.

## PR — A-CAT categoryId backfill terminal status (`chore/analytics-categoryid-terminal`) — NOT-ACTIONABLE
Decision-first close of the PR #99 additive consideration (`docs/internal/analytics-categoryid.md`). **Terminal status: NOT-ACTIONABLE**, three independent grounds: (A) `RebuildPopular`'s popularity window is **trailing-30d only** (`popularLookbackDays=30`), so category-less events age out within 30 days of #99's client going live — the gap is **self-healing**, a backfill's value lifetime is ≤30d and then overwritten by real data; (B) **nothing to backfill** — live-verified prod (2026-05-26 build) has no `analytics_schema.analytics_events` table at all (`to_regclass`→NULL), zero events; (C) **no category harmed** — handler falls back to global proxy (coherent bestseller list, never regresses to recommended), each category self-populates as events accrue. The §5-safe path (in-process `catalog.Service` call + same-schema UPDATE — analytics+catalog co-reside in core-svc; NO cross-schema JOIN) is documented as a blueprint but **not built** (unwarranted per A–C; §7-4 forbids backfill-by-default). Going-forward pipeline (#99) + ranking math (#100) untouched. Docs-only; no migration, no code.

## PR — F-019 reconcile grant + reconcile suite revival (`fix/f019-reconcile-grant`) — F-019 FIXED, F-018 10/10
Fixes the live prod bug F-018 surfaced: `init/73` granted reconcile_user **DELETE without SELECT** on `wallet_schema.event_delivery_attempts`; Postgres needs SELECT to evaluate the DELETE's WHERE predicate, so `CleanupOldAttempts` threw **42501 every weekly reconcile cron run** (reproduced on a fresh fixture). Fix: migration **0081_reconcile_select_grant** (existing DBs) + **init/73** (fresh DBs) converge on `GRANT SELECT, DELETE` — least-privilege (DELETE already present, SELECT was the only gap); additive, migration-safety green. **Reconcile suite revived (F-018 batch 2 → 10/10 closed)**: `integration-reconcile` env-pointer target on pg-ledger-test (the fixture applies init/* + migrations, so it carries the grant) + registered in verify; green under -race on a **fresh** torn-down/rebuilt fixture. Framing shift: reconcile had **no legacy self-spinning target** to delete (it was #103's "no target" group, not "colliding") — revival was purely additive; no TRUNCATE needed (suite manages its own rows). Live 42501 stops when 0081 rides the deploy-runway `ledger up`. Discovery: `docs/internal/f019-reconcile-grant.md`.

## PR — F-020 Home rails contract fix (`fix/f020-home-rails-contract`) — **F-020 CORRECTED**
The three server-driven Home rails (recommended / bestseller / newest) rendered empty. Surfaced by LOCAL-PHASEB-01. **Framing was partly wrong (Outcome C):** the prompt's `category_id`-required `400` was only the *first* of two blockers, and the deeper one is **app-wide, not rail-specific**. (1) `GET /products` hard-required `category_id` though the OpenAPI marks it `required:false` and documents `bestseller` as **global** popularity → fixed the handler to serve a global, catalog-wide `catalog.ListProducts` when `category_id` is absent (NULL-guarded `$1`, reusing `productSummarySelect`; validation kept when present; migration **0086** partial index `(created_at DESC) WHERE active` for global `newest`). That earned a `200` — **but the rails were still empty on the emulator.** (2) The rail provider used the **generated** `api.listProducts`, whose strict `ProductSummary.fromJson` requires `cashback_preview.monthly_coin_minor`, while the backend's shared `buildProductSummaryJSON` emits `monthly_amount_minor` (+ a `meta` envelope vs the spec's `pagination`) → `fromJson` **throws**, rail hides. This drift is **pre-existing and app-wide**: the **category PLP + search** use the same generated path and are likewise empty ("Nothing here yet"). The codebase already works around it for hand-written endpoints via `productSummaryFromApi` (its doc-comment names this exact trap); the working recommendations rail uses it, the naive `productsRailProvider` didn't. **Fix (rail scope):** repoint `productsRailProvider` to the raw `/products` response + `productSummaryFromApi` — mirroring the recommendations rail; backend response shape left unchanged. **Verified on the Android emulator:** all three rails populate (image + badge + price; bestseller correctly popularity-ordered, iPhone first); working rails untouched. New: handler unit test (`category_id` optional/global/400) + `TestIntegration_ListProductsGlobal`. **Filed F-021 (NEW):** the backend↔spec reconciliation that would also fix category PLP + search (make `/products` emit `monthly_coin_minor` + `pagination`, retire the manual mappers) — out of F-020 scope (touches working surfaces + spec/clients). Discovery: `docs/internal/f020-home-rails.md`. Local-verified; not deployed (deploys deferred to after Phase B+C).

## PR — F-021 /products ↔ spec reconciliation (`fix/f021-products-spec-reconciliation`) — **F-021 FIXED (atomic, Outcome A)**
Closes the app-wide drift F-020 surfaced: the catalog product-summary serializer diverged from its own OpenAPI, so the strict generated parse threw on **every** `/products`/`ProductSummary` consumer — **category PLP + search + PDP rendered empty**, and the app masked the rest with manual mappers. **Backend is the wrong side** (confirmed: the Dart **and web** generated types already use `monthly_coin_minor` + `pagination`; client code reads `.pagination`; the mappers' own doc-comments name the trap), so **no spec change, no client regen**. Two serializer edits in `cmd/core-svc/catalog_handlers.go`: `cashbackPreviewJSON` → the OpenAPI `CashbackPreview` shape (`monthly_amount_minor`→`monthly_coin_minor`; dropped the off-spec `reference_rate_bps`/`commission_pct_bps`, read by no consumer) and the list envelope `meta`→`pagination`. One struct fixes **8 endpoints** that share `buildProductSummaryJSON`/`buildProductListResponse` (`/products`, `/search`, `/sellers/{slug}/products`, `/home/flash-deals`, `/recommendations/home`, `/me/recently-viewed`, `/products/{id}/similar`, `/products/{id}` PDP). No value/logic change (cashback math untouched); the fin-svc cashback **plan** `monthly_amount_minor` column is a different, in-spec field, untouched. **Mobile:** retired both manual mappers (`productSummaryFromApi` + `sellerProductFromApi`), routing all 5 call-sites through the generated `ProductSummary.fromJson`; reverted the F-020 `products_rail_provider` workaround to generated `api.listProducts` (the F-020 **handler** global-list fix stays); PLP/filtered/PDP already on the generated path → fixed with **zero** mobile change. **Regression guards:** Go `TestF021_ProductListResponse_SpecKeys` (locks `pagination` + `monthly_coin_minor`, bans the old keys — the systemic root of F-020+F-021) + Dart `product_summary_parse_test` (generated `ProductSummary.fromJson` parses the shape; P-030 lowest_30d kept). **Verified:** curl confirms `/products`+`/search`+PDP emit `pagination`+`monthly_coin_minor`; on the emulator the rails render via the **generated** `api.listProducts` (the PLP's exact parse path) **with cashback MC-Coin amounts now displaying** (`monthly_coin_minor` parses) — PLP/search share that identical model. `make verify` green; flutter analyze / riverpod / i18n green. Discovery: `docs/internal/f021-products-spec-reconciliation.md`. Local-verified; not deployed. (Live PLP nav hit intermittent adb-reverse tunnel drops under heavy local load — host curl confirms the endpoint; not an F-021 regression.)

## PR — IA-01 Home restructure (`feat/home-ia-restructure`) — **IA-01 shipped (Outcome A)**
Two deliberate IA divergences from Trendyol (Salih's product direction), local-verified, not deployed. (1) Home's category section is now a **horizontal shortcut rail** (`HomeCategoryRail`, replacing the grid) ending in a **"Tüm Kategoriler"** puck → the full category tree. (2) The **Categories** bottom-nav tab became a **Coin** tab (`Icons.monetization_on`, `nav.coin`) → a routed `CoinScreen` placeholder (IA-02 builds the hub). The full tree is **preserved** and re-pointed from a `StatefulShellRoute` branch to a **top-level `/categories` route**, so deep-links keep resolving (home-rail `seeAllRoute`, invalid-id redirect, title rule) — Outcome A, no bounce-to-Home redirect needed. Discovery shift: the rendered nav is `_MobileShell` in `app_shell.dart` (the separate `BottomNavShell` is **dead code**, untouched); ~48 flat init+seed roots fill the rail. i18n `nav.coin`/`home.all_categories`/`coin.placeholder_*` (TR+EN), 0 dead / 0 missing. **Verified on the Android emulator:** Coin tab → "Mopro Coin" placeholder; category rail renders; a rail puck pushes the category route (data fetch hit the recurring adb-reverse tunnel flake — routing resolved). `make verify` green; flutter analyze / i18n / riverpod green. **Goldens:** 5 predicted flips (`home_goldens_5a` ×3 + `app_shell bottom_nav` light/dark) need **Linux** regen via `golden-rebaseline.yml` (macOS can't enumerate flips — the platform guard fails all Linux baselines locally); `app_shell_test` non-golden icon assertions updated in-tree. Also fixed F-021 fallout: 3 consumer test fixtures `monthly_amount_minor`→`monthly_coin_minor`. Discovery: `docs/internal/ia-01-home-restructure.md`.

## PR — IA-02 Coin hub (`feat/coin-hub`) — **IA-02 shipped read-only (Outcome C); redeem DEFER'd**
Fills the Coin tab IA-01 created. **Outcome C** (discovery): balance (`GET /wallet/balance`), earn/spend history (`GET /wallet/transactions`, credit/debit signed+dated), and cashback plans (`GET /cashback/plans` = ways-to-earn) all already exist and are wallet_schema-internal (**§5-safe**); the shipped `WalletScreen` already renders them. **No discrete redeem endpoint exists** — the only coin-spend model is the checkout `coin_balance` payment method (a payment flow, out of scope §1.2/§7.6) — so per §5/§7.3 **redeem is DEFER'd** (never half-build a financial mutation), surfaced as a "coming soon" card. **`CoinHubScreen`** (replaces IA-01's placeholder at `/coin`): balance header + recent earn/spend activity (shared `TransactionTile`) + ways-to-earn + redeem-coming-soon, reusing `walletProvider`/`coin_formatter` (no fabricated values); **soft guest-gate** (coins are per-user → login prompt for guests, vs a tab redirect); loading/empty/error; links to `/wallet` for full plans. **financial-core honored:** #3 soft-deleted-user guard (in `WalletSvc`, reused) + #7 no cross-schema JOIN apply to the reads; #1/#2/#4/#5 would govern the deferred redeem; the **dev seed** (`scripts/seed/data/coin-extras.sql`) writes **balanced double-entry (§4.1)** TRY_COIN credits+spend to dev user owner_id=1 (verified balance MV = 43059 minor ≈ 430 MC, 4 entries, ledger_balance_check satisfied), refreshing the balances MV. **Verification:** widget test `coin_hub_screen_test` (guest gate + authed balance/ways-to-earn/redeem/transactions render) green; `make verify` green; flutter analyze (coin files) / i18n (0 dead/0 missing) / riverpod green. Emulator screenshots avoided (image-API limit degraded the prior session) — widget test + the §4.1-correct seed are the render proof. **Goldens:** IA-02 adds **0** new flips (no golden test captures `/coin`); IA-01's bottom-nav (×2) + home (×3) goldens still await Linux regen (separate). i18n TR+EN; no backend/spec change, no client regen.

**⚠️ BLOCKER FOUND + handled on-branch (needs a main fix):** dependabot **PR #120 bumped `flutter_riverpod` 2.6.1 → 3.3.1** (major, breaking: `StateProvider` removed, `AsyncValue` API changed) **without migrating the codebase** → **785 analyze errors on main; the mobile app does not compile.** Pinned back to `^2.6.1` on this branch (build restored, 0 errors) to unblock IA-02 (per user direction). **Main still needs the same revert** (or a real 2.x→3.x migration as its own task) — don't auto-merge the riverpod major until then. Discovery: `docs/internal/ia-02-coin-hub.md`. Local-verified; not deployed. **→ Resolved by F-022 below.**

## PR — F-022 Main dependency stability (`fix/riverpod-pin-dependabot-hardening`) — **main green; Dependabot hardened**
Closes the riverpod blocker IA-02 surfaced and the systemic gap that let it land. **Discovery shift (escape hatch §1.3): main was already green** — the IA-02 pin (`7aa9f4d4 chore(mobile): pin flutter_riverpod to ^2.6.1`) **carried back onto `main` via the #124 coin-hub merge**, so `main:mobile/pubspec.yaml` is `^2.6.1` (lock resolves **2.6.1**); `flutter analyze` = **0 errors / 0 warnings / 199 infos** (the infos are pre-existing `very_good_analysis 10.2.0` debt, DEFER'd — not compilation failures). So **§3.1 was a confirmation, not a re-pin** — no pubspec edit needed. **PR #120 is MERGED** (2026-06-08; bumped to 3.3.1), with the pin-revert landing *after* it; no other open Dependabot PRs. **Root cause (the real fix): Flutter CI is not a required status check.** Branch protection on `main` requires only `verify` (make-verify.yml, which does NOT run `flutter analyze`); `flutter-ci.yml`'s `flutter analyze` job is **not required**, so #120 went red on analyze yet still merged. **Hardening shipped:** `.github/dependabot.yml` now **ignores ALL `pub` semver-majors** (`dependency-name: "*"`, `version-update:semver-major`) so a breaking major can no longer be auto-proposed/merged for the Flutter app until migrated deliberately — this prevents 3.3.1 re-landing; minor/patch still flow (grouped). No auto-merge existed to disable. **DEFER'd:** (a) the real riverpod **2.x→3.x migration** as its own task (`StateProvider`/`AsyncValue` rewrite — never half-migrate); (b) the **very_good_analysis 10.2.0** ~199 analyze-infos tech-debt. **Salih repo-settings action (not code):** make **`flutter analyze`** (the `flutter-ci.yml` job name) a **required status check** on `main` — Settings → Branches → require status checks → add `flutter analyze` (+ optionally `flutter test`, `build_runner …`); API: `gh api -X PATCH repos/:owner/:repo/branches/main/protection/required_status_checks -f strict=false -f 'contexts[]=verify' -f 'contexts[]=flutter analyze'`. This closes the systemic root so a non-compiling PR can't merge again. Discovery: `docs/internal/f022-deps-stability.md`. Local-verified; not deployed.

## PR — F-022b Analyze green-on-compile + required check (`chore/flutter-ci-analyze-required`) — **CI green; required-check command ready for Salih**
Completes F-022's protection. F-022 named the root cause but the `flutter analyze` job stays **red** because it runs a bare `flutter analyze` (the 199 `very_good_analysis` infos are fatal by default) — so it could not be required, and a non-compiling PR could still merge with only `verify` gating. **§3.1 (the only code change):** `.github/workflows/flutter-ci.yml:32` → `flutter analyze --no-fatal-infos`. Confirmed locally: bare `flutter analyze` exits **1**, `--no-fatal-infos` exits **0**; main breakdown is **0 errors / 0 warnings / 199 infos** — so errors+warnings stay fatal (a non-compiling PR still fails) and only infos are tolerated (anti-goal #1 honored; no warnings to fix → §5 split not triggered). The 199 infos remain **DEFER'd** (anti-goal #2). **Exact PR check-context name:** **`flutter analyze`** (verbatim, via `gh pr checks`). **Current `main` protection:** `required_status_checks = { strict: false, contexts: ["verify"] }` — only `verify`. **Salih repo-settings action (the actual gate-close), to apply ONLY after this PR merges and `flutter analyze` is confirmed green on `main`:**
```bash
gh api --method PATCH \
  repos/s4l1hs/Mopro-Shop/branches/main/protection/required_status_checks \
  -F strict=false \
  -f 'contexts[]=verify' \
  -f 'contexts[]=flutter analyze' \
  -f 'contexts[]=flutter test' \
  -f 'contexts[]=build_runner (verify generated files up-to-date)' \
  -f 'contexts[]=i18n completeness (extras gate)' \
  -f 'contexts[]=i18n dead-key gate' \
  -f 'contexts[]=riverpod inference gate' \
  -f 'contexts[]=dart analyze (mopro_api generated client)'
```
The **minimum** to close the F-022 hole is `verify` + `flutter analyze`; the rest are all green on PR #137 and require-ready (recommended). **Gotchas:** (1) the PATCH **replaces** the whole contexts list — omitting `verify` would silently drop it (keep it in the list). (2) A required context whose name doesn't **exactly** match a reported check leaves every PR stuck "Expected — Waiting for status to be reported" → use the verbatim strings above. (3) Apply only **after** merge + analyze green on `main` — requiring a currently-red check blocks all merges. `strict: false` preserved (no "branch up-to-date" requirement). Discovery: `docs/internal/f022b-analyze-required.md`. CI-verified; not deployed.

## PR — Trendyol Home parity audit (`docs/trendyol-parity-home-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_HOME_AUDIT.md`: a structured parity audit of the **restructured** Home (post-IA-01) against the provisional Trendyol baseline. **Audit-only — no code changed** (audit-then-fix separation). Self-audit from source (10-row §3 table, `home_screen.dart`/`product_card.dart`/`home_category_rail.dart`/`flash_deals_rail.dart`/`app_shell.dart`/`tokens.dart`): **6 PROBABLE deltas** (HP-01 no "Sepette %X" basket-discount pill, HP-02 no per-card "Çok Satan" bestseller badge, HP-03 category pucks rounded-square not circular, HP-04 no camera/visual-search icon, HP-05 no header location selector, HP-06 no header notif bell — all Mopro-side confirmed from source, Trendyol-side general-knowledge → PROBABLE, no severity until walked); **5 NOT-ACTIONABLE intentional divergences** (D1 Coin tab, D2 categories-as-rail, D3 cashback chip, D4 coin-balance pill, D5 Mopro brand-orange token); **9 already-matched** (banner carousel + dots, flash-deals + countdown, for-you/bestseller/rec rails + see-all, card anatomy 1:1/discount-pill/strikethrough/brand-bold/heart/rating/free-shipping, mood-stories, trust bar, TR microcopy, responsive). Walk-findings slots in #09 finding format (§6) ready for Salih to paste live-Trendyol observations + flip PROBABLE→CONFIRMED + assign severity; new findings as HP-07+. **Discovery shifts:** the restructured Home is already Trendyol-shaped — open deltas are card-merchandising pills + header chrome, not structural; the two highest-value-if-confirmed are HP-01/HP-02 (both likely backend-signal-gated); HP-03 is the only zero-dependency pure-UI candidate (golden-flip). Awaiting walk → then per-finding fix prompts (CONFIRMED-HIGH/MED first). No deploy.

## PR — Trendyol PLP/category-browse parity audit (`docs/trendyol-parity-plp-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_PLP_AUDIT.md`: structured parity audit of the **PLP** (`CategoryProductsScreen` + `CatalogShell`/`FilterPanel`/`FilterSheet`/`PlpFilterChips`/`sort_sheet`/`filtered_products_provider`; same shell backs SearchScreen). **Audit-only — no code changed.** Self-audit from source: **11 PROBABLE deltas** (PLP-01 mobile filter sheet lacks brand+rating — desktop-only; PLP-02 applied-chips desktop-only; PLP-03 manual "Load more" not infinite-scroll; PLP-04 no visible result count though `pagination.total` exists; PLP-05 no visible breadcrumb (JSON-LD only); PLP-06 no quick-filter pills; PLP-07 brand facet no counts + derived from loaded page; PLP-08 no clear-filters CTA on no-results; PLP-09 no fast-delivery toggle; PLP-10 no PLP-header search bar; PLP-11 in-stock on mobile sheet but missing desktop sidebar); **4 NOT-ACTIONABLE** (D1 cashback sort/filter, D2 URL-synced shareable filters + SEO JSON-LD = Mopro PLUS, D3 brand-orange tokens, D4 PLP share button); **~10 already-matched** (desktop searchable-brand sidebar, price slider+fields, rating buckets, free-shipping, 6-option sort dropdown+sheet, responsive 2/3/5 grid reusing the parity'd card, removable applied-chips, load-more w/ spinner+retry, empty+error states, live filtering). Highest-value-if-confirmed: **PLP-01** (mobile can't filter brand/rating) + **PLP-03** (infinite scroll). Walk-slots in #09 format (§6). **Seed caveat (§5):** global variety rich (50 products / 25 brands / ₺89–89,999 / 11 ratings) but **per-category density is 2–3 products** → a single category PLP can't exercise the filters; flagged a tiny ~20–30-product one-category seed concentration (or confirm parent-category subtree rollup) as its own task — not built here. Awaiting walk → per-finding fix prompts. No deploy.

## PR — PLP audit reconciliation (`docs/plp-audit-reconciliation`) — **canonical registry; markup folded**
Turned `docs/audits/TRENDYOL_PARITY_PLP_AUDIT.md` into the **single canonical PLP registry**: one stable ID per finding, each tagged **evidence source** (`src`/`walk`/`markup`) + **confidence**. **Audit-doc only — no code.** **Drift resolved (§2 alias map):** the contract/ledger's `PLP-02/05/07` (sticky sidebar / ultra-wide grid / sticky mobile bar) collided with the audit's `PLP-02/05/07` (chips / breadcrumb / brand-counts) → contract ones re-numbered **PLP-18/19/20**, audit numbers kept; contract "PLP-25" recorded as the alias for the shipped PLP-01+03 bundle. **Markup folded** (homepage + `/cep-telefonu`; `/sr` 403 → search needs Salih's eyes): **CONFIRMED structural** — PLP-04 result count ("3966+ Ürün"), PLP-05 breadcrumb trail, **PLP-12 subtree rollup** (HIGH), **PLP-13 attribute facets** (HIGH, new), PLP-09 fast-delivery. **New from markup:** PLP-13 (attribute/variant facets), PLP-14 (price-history *filter*, distinct from on-card lowest-30d), PLP-15 (desktop **numbered pages** — corrects the old "infinite scroll desktop" assumption; split from PLP-03 mobile), PLP-16 (ranked bestseller badge), PLP-17 (official-seller badge). **Softened:** PLP-07 brand-counts (not in SSR → inconclusive). **Kept PROBABLE** (visual, await walk): mobile sheet styling, hover chevrons, banner indicators, sticky-sidebar/grid/bar (PLP-18/19/20). **CONFIRMED-HIGH fix queue (§8):** PLP-13 attribute facets → PLP-12 rollup → PLP-04 count → PLP-05 breadcrumb → PLP-15 numbered-pages → PLP-14 price-history. **PLP-01/03 (#142) unaffected** (walk-confirmed, desktop-untouched). Cross-updated `CUTOVER_LEDGER.md` §4/§7 for the ID re-map. No deploy.

## PR — Trendyol Search parity audit (`docs/trendyol-parity-search-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_SEARCH_AUDIT.md`: parity audit of Search (`SearchScreen` + `searchProvider` + `SearchSuggestionsDropdown` + `recent_searches_provider` + backend `SearchProductsSummary`). **Audit-only — no code.** `/sr` is 403 → no Trendyol markup; leans on the impl self-audit (`src`) + the walk. **Headline (PLP-inheritance check, §2): PARTIAL** — `SearchScreen` reuses the shared PLP *widgets* (`CatalogShell` grid, desktop `FilterPanel`, `PlpFilterChips`, sort, **PLP-20 sticky bar**, **PLP-18 sticky sidebar**, empty/error) but has its **own** `_results`/`_shell` wiring, so it **did not** inherit the screen-level PLP wins: **SE-02 mobile search has no filter affordance** (`onFilter` unset → sort-only on mobile), **SE-03 no result count** (`SearchState` has no `total`), **SE-04 load-more on both** (no infinite-scroll/numbered-pages), **SE-05 flat breakpoints** (not 2/3/4/5). Search-specific deltas: **SE-06** dropdown lacks brand/product autocomplete (has recent+trending+category), **SE-07** no-results = generic empty (no "did you mean"/alternatives), **SE-08** results order by sort token not ts_rank relevance (PROBABLE), **SE-09** mobile empty lacks trending, **SE-10** no search-within/sponsored (PROBABLE). **4 NOT-ACTIONABLE** (no camera HP-04, no sponsored ads, cashback chip, brand tokens). **~8 already-matched** (bar+entry points, recent-search persistence, trending, category suggestions, the inherited shared widgets, query echo, debounce). **Seed exercisable** (nike=6, ayakkab=2, iphone/apple=6). **Discovery shift:** "inherited from PLP" is true only for shared widgets — the screen-level wins must be *ported* to `SearchScreen` (same fixes; cheapest = extract a shared results body); PLP-13/14 facets auto-surface in the shared `FilterPanel` once built. Walk slots (#09) for the search-specific bits (suggestions/no-results/relevance). Awaiting walk → per-finding fix prompts. No deploy.

## PR — Trendyol PDP parity audit (`docs/trendyol-parity-pdp-audit`) — **seeded; seed-extension prerequisite flagged**
`docs/audits/TRENDYOL_PARITY_PDP_AUDIT.md`: parity audit of the product detail page (`ProductDetailScreen` + `PdpImageGallery`/`PdpImagePager` + `PdpPriceBlock` + `PdpVariantSelector` + `PdpStickyCta` + `PdpDeliveryInfo` + `PdpSellerCard` + `PdpReviewsTab`/`RatingDistributionHistogram` + `PdpQaTab` + similar rail). **Audit-only — no code.** **The PDP is largely built.** **#77–#103 confirmed MATCHED:** P-007 delivery-ETA, P-030 lowest-30d, P-032 strikethrough (+ P-015 OOS variants). **CONFIRMED gaps (src):** **PD-01 the specs/attributes tab is a STUB** (`_StubTab`/loading — HIGH, **ties PLP-13** attribute model), **PD-03** no basket-discount "Sepette %X" on the PDP price block (data on the product, unsurfaced), **PD-07** reviews have histogram+sort+helpful but **no photo reviews**. **CONFIRMED-src delta:** **PD-04** seller card has no rating/official badge (**ties PLP-17**). **PROBABLE (walk):** PD-02 variant pickers are text `FilterChip`s not colour swatches/size pickers, PD-05 no installments (cashback card is the analog — confirm if substitute), PD-06 gallery (no video; mobile has no thumbnail strip — desktop has thumbnails+hover-zoom), PD-09 desktop buy-box not pinned (gallery sticky-translates). **3 NOT-ACTIONABLE** (coin/cashback card, brand tokens, trust badges vs verbose cargo/return). **🚩 Headline — SEED INADEQUATE FOR THE WALK:** live DB shows **1 image/variant** (no gallery), **0 products with >1 variant** (selector never renders), **0 reviews** (tables exist, empty) → gallery, variant selection, and reviews are **not exercisable**; a **PDP seed extension (multi-image + multi-variant incl. OOS + reviews) is a prerequisite task** before the walk. **Cross-surface ties:** PD-01/PD-02 ride the PLP-13 attribute model; PD-04 rides the PLP-17 official-seller flag — those backends light up PLP *and* PDP. Walk slots (#09). Awaiting seed extension + walk. No deploy.



## PR — Trendyol Checkout parity audit (`docs/trendyol-parity-checkout-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_CHECKOUT_AUDIT.md`: parity audit of the auth-gated Checkout flow (`CheckoutStepper` → `CheckoutAddressScreen` → `CheckoutPaymentScreen` → `CheckoutReviewScreen` → `CheckoutResultScreen` + 3DS/Sipay webviews). **Audit-only — no code.** **Checkout is built:** 3-step flow, address select/add/**default**, card (**3DS PSP-hosted**) + bank-transfer payment, **TR-legal consent** (distance-sales + pre-info) gating place-order, result/confirmation (order numbers, cashback note, clears cart). **CONFIRMED gaps (src):** **CHK-01** review summary shows **total only** (no subtotal/shipping/KDV/cashback breakdown — 🔗 Cart's `totalsBySeller` unused here); **CHK-02** **no per-seller grouping** in review (🔗 Cart); **CHK-03** **no delivery options/slots** step. **🧩 Shared backend cluster (lands on cart + checkout — build once):** CHK-04 coupon (=CT-03), CHK-05 basket-discount (=CT-09), CHK-06 free-shipping (=CT-02). **PROBABLE (walk):** CHK-07 saved-cards/installments (PSP-hosted; cashback is Mopro's analog), CHK-09 validation/error states. **5 NOT-ACTIONABLE:** coin-redeem-as-payment (`cashback` method present but **`enabled: false`** — deferred IA-02), cashback-earned note, PSP-hosted card entry, no installments, brand tokens. **Cart-inheritance:** CHK-01/CHK-02 are the *same* fixes as Cart's CT-04/CT-01 (#173) — the review just didn't inherit them. **Flow/seed:** reachable locally to **address→payment→review** (auth + cart + address); the **3DS Sipay charge needs sandbox creds**, coin-redeem disabled. **Cascade:** the fix batch builds the shared backend cluster (coupon/basket-discount/free-shipping) once, benefiting Cart too. Walk slots (#09). No deploy.

## PR — Trendyol Account parity audit (`docs/trendyol-parity-account-audit`) — **seeded; read-path-checked; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_ACCOUNT_AUDIT.md`: parity audit of the Account hub (`account_screen` + desktop `account_left_rail`/`account_right_pane`/`account_shell` + sub-screens profile/cards/security/privacy/my-reviews/my-questions/browsing-history + the coin `wallet/`). **Audit-only — no code.** **Headline methodology — the read-path reality check:** every section rated **L** live / **S** stub / **U** UI-only by verifying the *backend actually serves it* (not just that a widget exists) — the correction for the cart audit's false "matched" over a stub backend. **Result: 13 LIVE / 2 STUB.** The hub passes the reality check far better than the cart — most sections read a real backend (verified to the handler: `GET /me`, `GET /orders`, fin-svc `GetWalletBalance`→`WalletSvc.GetBalance`+transactions+plans, `/addresses` CRUD→`identity.Service`, `/me/reviews`, `/me/questions`, `/me/password`+`/auth/mfa`, `/me/consent`+`/me/analytics-data`, `/me/recently-viewed`). **The 2 stubs the check caught (a UI-only pass would have marked "matched"):** **AC-01 Saved Cards** — `cards_screen` always renders `cards.empty` + add-card is `// TODO(mopro): wire to add card flow` (PSP-hosted model softens it, but the section is dead); **AC-02 Help** — the hub Help entry is **`onTap: () {}`** (dead button) though `internal/help`/`internal/support` backends exist (unwired) — the cheapest real fix. **2 CONFIRMED gaps:** AC-03 no email-change in Security (password + phone/MFA only); AC-04 no notification-preferences setting. **PROBABLE:** AC-05 no membership tier/points (coin + active-plans stats instead). **3 NOT-ACTIONABLE divergences:** the Coin/cashback wallet hub (vs Trendyol coupons/wallet, IA-02), coin+plans stats (vs Elite tier), brand-orange tokens; PSP-hosted cards is the intentional *model* (the dead stub AC-01 is the flagged part). Walk slots (#09) in §5; fix list (§6) puts the stubs first (AC-02 wire-help is the cheapest). **Discovery shift:** the read-path check is the reusable corrected pattern — it converted "looks complete (10+ live-seeming entries)" into "2 dead entries" objectively. Awaiting walk. No deploy.

## PR — Trendyol Orders parity audit (`docs/trendyol-parity-orders-audit`) — **seeded; read-path-checked; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_ORDERS_AUDIT.md`: parity audit of Orders (history + detail) (`order_history_screen` + `order_detail_screen` + `OrderStatusTimeline`/`CashbackSchedule`/`order_eligibility_actions`/`refund_status_card`; returns = own audit). **Audit-only — no code.** Applied the **read-path reality check** (L live / S stub / U UI-only / **A absent**). **Headline: the Orders core is LIVE with ZERO stubs** — verified to the backend (`handleListOrders`/`handleGetOrder` → `order.Service` → `pgxOrderRepository`): the list, detail, status timeline (derived from status + `shipped_at`/`delivered_at`), totals breakdown (items/shipping/commission/kdv), `actions` (canCancel/canReturn), `RefundInfo`, and the cashback schedule all read real data. **This is the clean case** — contrast Account (Help/Cards dead-button stubs) and Cart (whole read-path stub). The Orders gaps are **honest A-absences** (features not built), a healthier class than stubs: **OR-01** no cargo tracking (carriers push via `POST /shipping/webhook/*` but carrier/tracking_no isn't surfaced; timeline is status-derived, not "where's my package"), **OR-02** no delivery address on the order, **OR-03** no invoice/e-arşiv (`internal/einvoice` PLANNED), **OR-04** no reorder, **OR-05** no variant label on order-item lines (`OrderItemDto` has `variantId` only — the same pre-enrichment gap the cart had; the `GetVariantByID` carrier already exists), **OR-06** no server search/pagination (`ListOrders(userID)` returns all; filter/search are client-side). **3 NOT-ACTIONABLE** (cashback schedule on order + refund-as-coin `wallet_credit` = Mopro model; brand tokens). Fix list (§6) is "build-the-absent-feature" (no stubs to wire) — cheapest are OR-05 (reuse the cart enrichment carrier) + OR-04 (re-add items to cart). Walk slots (#09) §5. **Discovery shift:** the read-path check distinguishes a 4th rating — **A (absent/honest gap)** vs **S (stub/dead UI)** — and Orders is the first surface that's all-L-core + A-gaps, no S. Awaiting walk. No deploy.


## PR — OR-02 order delivery-address capture (`feat/order-address-capture`) — **shipped (local-verify; deploy deferred)**
Closes **OR-02** (the deferred Orders gap): orders now capture the delivery address as an **immutable as-of-purchase snapshot** and render it on the detail. Discovery (`docs/internal/order-address.md`) confirmed the address was known **only client-side** — checkout split `Address.name` into the PSP buyer name and dropped the `address_id`. **Fix (per-concern commits):** (1) migration **0093** `order_schema.order_addresses` (order_id PK FK→orders, **AES-GCM-encrypted** recipient_name/phone/full_address/neighborhood to match `identity_schema.addresses` §6; district/city/postal_code/label plaintext for logistics) — additive, legacy orders have no row; (2) `order.OrderAddress` + `Order.DeliveryAddress` (read-path only — `service.GetOrder` attaches it; financial `repo.GetOrder` callers untouched) + `InsertOrderAddress`(encrypt)/`GetOrderAddress`(decrypt) via `pkg/crypto`; (3) **capture** — `address_id` on `InitiateCheckoutRequest`, resolved **once before the persist tx** via the in-process **`order.AddressResolver`** (§3.1; `cmd/core-svc` adapts `identity.Service` so `internal/order` stays decoupled — **no order→identity import**) and frozen onto each per-seller order **inside the same tx** (atomic + once-only with the order's session idempotency); resolve failure is non-fatal, no `address_id` → no snapshot (legacy-safe); (4) spec `DeliveryAddress` on `Order` + regen Go/Dart + **live-handler contract test** (`GET /orders/{id}` surfaces `order.delivery_address`, validated against the `DeliveryAddress` schema — scoped to the field OR-02 owns; the legacy `Order` serialization's pre-existing drift, e.g. `status:"paid"` absent from the spec enum, is untouched); (5) mobile — `DeliveryAddressDto` + `DeliveryAddressCard` on the detail (rendered only when a snapshot exists), checkout sends `address_id` from `selectedAddress.id`, i18n `order.delivery_address_title` (tr master + en), DTO+widget tests. **§5-safe** (snapshot, not cross-schema FK/JOIN); boundary green; no goldens flip (card is conditional; no existing order-detail golden carries an address). Audits updated (`TRENDYOL_PARITY_ORDERS_AUDIT.md` OR-02 → RESOLVED). **Discovery shifts:** the address lived only on the client (the audit's "needs a checkout-capture vertical" confirmed); PII must be **re-encrypted** in the order snapshot (not just copied) to keep §6; the order package resolves the address through a neutral resolver interface so it never imports identity. Local-verify only; **no deploy** (the migration runs on the deploy-runway `ledger up`/`ecom up`). Unblocks the heavier Orders items needing an address (OR-01 cargo, OR-03 invoice).



## PR — PD-05 installments (taksit) at checkout (`feat/installments`) — **shipped, INTEREST-FREE (Salih-confirmed); no money-math change**
The financial/PSP-gated lane, discovery-first. **Discovery** (`docs/internal/installments.md`): Sipay `paySmart3D` supports installments and our integration **anticipated it** — `SignPayment3D` models the documented `Installment` hash field — but the live `initPayReq` sent **neither `installments_number` nor `hash_key`** (never threaded). **Discovery shifts:** (a) **per-BIN plan listing (`getpos`) is architecturally unavailable under SAQ-A** — the card is typed on Sipay's hosted form *after* initiation, so Mopro never sees the BIN → the picker is the **generic 1/3/6/9/12 set**, with unsupported card/count combos **bank-rejected inside 3DS** (degrades exactly like any failed 3DS; saga compensation already handles it); (b) buyer-facing **vade-farkı would break the asserted display==charge invariant** (webhook SHA-512 over `total_amount` + capture ledger + refund/cashback baseases) → the **funding-model gate went to Salih, who confirmed INTEREST-FREE** → zero money-math change anywhere (the charged `total_amount` is untouched; PSP per-count commission stays a Sipay merchant-panel concern). **Implementation (per-concern):** (1) backend — `payment.InitiatePaymentRequest.Installments` + `NormalizeInstallments` ({1,3,6,9,12}, 0→1) + `ErrInvalidInstallments`; Sipay now sends `installments_number` **+ the documented `hash_key`** (`SignPayment3D`, finally wired); order: `installments` on the hand-written `/checkout/initiate` body → saga (**validated BEFORE any persistence** → clean 422, no half-created session) → recorded on `order_schema.checkout_sessions` (**migration 0094**, additive DEFAULT 1 — the payments row is born at webhook time from fields that don't echo the count, so the session is the as-initiated record) → threaded to the PSP. (2) mobile — `_InstallmentPicker` ChoiceChips under the **card** method only (+ interest-free note), `CheckoutState.installments` (default 1, unsupported ignored), repo sends `installments` only when >1; i18n `checkout.installments_{title,single,n,note}` tr+en (strict 0/0). **No codegen** — `/checkout/initiate` is the established hand-written endpoint (favorites/reviews/OR-02 precedent). **Tests:** adapter (threaded+hash exact-match, default→"1", invalid-pre-PSP), saga (threaded to PSP+session, **full-total invariant** psp==session amount, default, invalid-before-persistence), controller (default/select/ignore/thread), picker widget (render/tap/hide-for-non-card) — order+payment suites green; money paths untouched by construction (no order/orderledger/cashback/refund math edits). Audits: PDP **PD-05 → RESOLVED** (checkout-side; PDP messaging stays the cashback card) + checkout **CHK-07 installments ✅** (saved cards stay PSP-hosted N-A). **Cannot be sandbox-verified blind** (no Sipay creds — pre-existing constraint); request shape/hash/recording are test-locked. No deploy.

## PR — Web platform consolidation onto Flutter (`feat/web-flutter-consolidation`) — **ADR + code; deploy cutover gated**
Decommissioned the parallel Next.js/TypeScript `web/` app and made the **Flutter app the single UI for mobile AND web** (ADR-0005). **Why it was low-risk:** the Next.js app was **never deployed** — `deploy/caddy/Caddyfile` served `moproshop.com` as a `"Çok yakında / Coming soon"` placeholder, no reverse-proxy to a Next process anywhere; and the Flutter client (shared mobile/web codebase) already calls the Go backend directly (Caddy → core-svc/fin-svc), so the Next.js BFF routes (`web/app/api/payments|auth/*`) had **no consumer** and needed **no migration**. **Changes:** (1) **ADR-0005** documents the decision + the accepted SEO/SSR trade-off (Flutter Web is client-rendered; mitigations = existing `seo_head`/`structured_data_service_web` runtime injection + sitemap/robots + a future prerender escape hatch only if Search Console regresses); (2) validated **`flutter build web --release`** succeeds (48 MB bundle, `build/web/index.html`); (3) **Caddy** now `file_server`s the build at `moproshop.com`/`www` (SPA fallback to index.html) + a dev `web.localhost` block — `caddy validate` green; (4) **compose** mounts `../mobile/build/web:/srv/web:ro` into Caddy; (5) `make web-build` target; (6) **removed** the entire `web/` Next.js app (~154 files) + the web-only `e2e.yml` Playwright workflow; (7) **CLAUDE.md** §2.2 + §8 updated — "all client UI is Flutter only," web = `flutter build web`, TypeScript/Next.js struck from the stack; (8) stale docs reconciled (p033 analytics no-op, parity-audit route mirror, L9c/L10 launch gates, integration-tests-wiring). **The "make web look like Trendyol" goal is already satisfied** — the Flutter app is the Trendyol-parity surface the whole project has been building, now served as the web storefront too. Gates: `flutter build web` ✓, `caddy validate` ✓, `docker compose config` ✓, boundaries ✓, build artifact gitignored. **Deploy cutover is gated/phased** (CI must ship `mobile/build/web` to the VDS `/srv/web` + flip `moproshop.com` from placeholder; monitor SEO post-cutover) — consistent with the project's local-verify-deploy-deferred norm. Branch: `feat/web-flutter-consolidation`.
