...
## PR — Trendyol Favorites parity audit (`docs/trendyol-parity-favorites-audit`) — **seeded; awaiting Salih's walk**
`docs/audits/TRENDYOL_PARITY_FAVORITES_AUDIT.md`: parity audit of the Favorites surface (`favorites_screen.dart` `FavoritesScreen`/`_favProductsProvider` batch-fetch/`_EmptyState`/`_SkeletonGrid` + `favorites_provider.dart` SharedPreferences `Set<int>` + the reused `ProductCard` + `POST /favorites/sync`/`catalog_schema.user_favorites`). **Audit-only — no code.** Favorites is auth-gated on Trendyol → **not SSR-fetchable**, so the Trendyol side is PROBABLE pending Salih's logged-in walk. **Headline (card-inheritance check, §4): CARRIES** — the grid renders the already-parity'd `ProductCard`, so image/brand/title/price/discount+strikethrough+lowest-30d/cashback chip/free-shipping+bestseller+**official-seller (PLP-17, just shipped #169)** badges/favorites-count overlay/heart-toggle all inherit; no favorites-only card gap. **Favorites-specific findings: 7** — CONFIRMED (src): **FAV-02** one-way sync (the substantive one — `POST /favorites/sync` upserts local→server on login but there's **no `GET /favorites`**/server→local hydration; the mobile list reads SharedPreferences only and the server `user_favorites` rows feed *only* the aggregate P-004 count → **no cross-device favorites**, MED–HIGH), **FAV-01** flat list / no collections (= flagship **P-013**, MED), **FAV-03** hardcoded `'Temizle'` clear-all (not `.tr()`, = subset of **P-014**, LOW), **FAV-04** error→infinite-skeleton (both `error:` and empty-`data:` fall to `_SkeletonGrid`, no retry, LOW); PROBABLE (await walk): **FAV-05** no add-to-cart on the card, **FAV-06** no sort/filter, **FAV-07** no "fiyatı düştü since favorited" indicator (favorites store only IDs — no price-at-favorite snapshot). **2 NOT-ACTIONABLE** (D1 guest-local favorites — no login wall, a deliberate guest-friendly Mopro choice arguably *better* than Trendyol's auth-gate, distinct from the FAV-02 down-sync gap; D2 cashback chip). **No seed needed** — favorites populate client-side (tap any heart); the grid/empty/remove flows are exercisable locally, sync-up by logging in, the down-sync gap observed on a second device. **Discovery shift:** the surface is cheap precisely because the card carries it — the only substantive finding is the architectural one (FAV-02 one-way sync); FAV-03/FAV-04 are trivial UI fixes. ID-reconciled with the flagship audit (FAV-01=P-013, FAV-03⊂P-014). Walk slots (#09) in §6. Awaiting walk → per-finding fix prompts. No deploy.
