...
func TestResolveCoupon(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil coupon", nil, 10000, 0, false, "not_found", 0},
		{"valid 10pct", base, 10000, 0, true, "", 1000},
		{"valid rounds half up (buyer-favour)", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: now.Add(-time.Hour)}, 105, 0, true, "", 11}, // (105*10+50)/100 = 11
		{"inactive", &Coupon{Code: "X", PercentOff: 10, Active: false, StartsAt: now.Add(-time.Hour)}, 10000, 0, false, "inactive", 0},
		{"not started", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: future}, 10000, 0, false, "not_started", 0},
		{"expired", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: now.Add(-time.Hour), ExpiresAt: &exp}, 10000, 0, false, "expired", 0},
		{"below min basket", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: now.Add(-time.Hour), MinBasketMinor: 20000}, 10000, 0, false, "min_basket", 0},
		{"exhausted", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: now.Add(-time.Hour), MaxRedemptions: ptrInt(5)}, 10000, 5, false, "exhausted", 0},
		{"under max ok", &Coupon{Code: "X", PercentOff: 10, Active: true, StartsAt: now.Add(-time.Hour), MaxRedemptions: ptrInt(5)}, 10000, 4, true, "", 1000},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := resolveCoupon(tc.coupon, tc.subtotal, tc.redemptions, now)
