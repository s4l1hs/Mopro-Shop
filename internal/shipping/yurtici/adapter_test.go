...
// buildServer returns a minimal SOAP server for TrackShipment.
...
func TestAdapter_TrackShipment_Delivered(t *testing.T) {
	...
	a := yurtici.New(shipping.YurticiConfig{WSDLURL: srv.URL, Username: "u", Password: "p"})
	res, err := a.TrackShipment(context.Background(), "YI-001")
	if err != nil {
...
func TestAdapter_HandleWebhook_NotSupported(t *testing.T) {
	a := yurtici.New(shipping.YurticiConfig{})
